...
public class ForcePlacingToolItem extends BlockToolItem implements InteractsWithEntity, RendersBeforeOutline, ItemResourceGenerator {
    ...
    private static boolean hasAccess(PlayerEntity player, World world, boolean placing) {
        ...
        if (MishangucRules.isAllowedToUse(player, placing)) {
            return true;
        }
        return false;
    }
    ...
    @Override
    public void generateBlockStateModels(Models models) {
        // 无需为此物品生成模型。
    }

    @Override
    public void generateItemModels(ModelJsonBuilder modelJsonBuilder) {
        // 无需为此物品生成模型。
    }
}
